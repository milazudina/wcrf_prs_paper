...
df_ukbb_old_cancers = read_excel("~/Dropbox/PRS_clean_run/_deprecated_prs_24_08_21/ukbb_prs_24_08_21.xlsx", sheet = 3)

colnames(df_ukbb_old)
colnames(df_ukbb_old_cancers)
colnames(df_ukbb)
...
df_ukbb_old_cancers = df_ukbb_old_cancers %>% 
  select(colnames(df_ukbb_old_cancers)[colnames(df_ukbb_old_cancers) %in% colnames(df_ukbb)])

df_ukbb_old = rbind(df_ukbb_old, df_ukbb_old_cancers)
...
Cancer PRS: All associations except pancreatic cancer PRS lose significance

``` {r cancer-prs-ethnicity, echo=FALSE, fig.show="hold", out.width="50%"}

df %>% 
  filter(Weighting == "Weighted", Outcome == "T2D", grepl("cancer", SNPs), Adjustment == "6 PCs") %>% 
  ggplot(., aes(x = SNPs, y = OR, ymin = lower_CI, ymax = upper_CI, color = Cohort))+
  geom_point(position = position_dodge(0.7),
             cex = 2)+
  geom_errorbar(lwd = 1.25,
                width = 0.3,
...
if (show_unweighted){
  ...
  df %>% 
  filter(Weighting == "Unweighted", Outcome == "T2D", grepl("cancer", SNPs), Adjustment == "6 PCs") %>% 
  ggplot(., aes(x = SNPs, y = OR, ymin = lower_CI, ymax = upper_CI, color = Cohort))+
  geom_point(position = position_dodge(0.7),
             cex = 2)+
  geom_errorbar(lwd = 1.25,
  ...
  theme(axis.title.x = element_text(vjust=-0.5, size = 12.5),
        ...
        axis.title.y = element_text(size = 12.5),
        axis.text.y.left = element_text(size = 12.5))+
  labs(title = "Unweighted Cancer PRSs - T2D outcome",
       x = "PRS")
